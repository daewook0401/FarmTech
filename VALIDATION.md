...
나무 벌목·우클릭 수확 추가 검증 (2026-09-13):

- 클라이언트와 서버에 HT’s TreeChop 0.19.0 fixed, Panda’s Falling Trees 0.13.2, PandaLib 0.5.2, RightClickHarvest 4.6.1+1.20.1, JamLib 1.3.6+1.20.1-patch.1 추가. 양쪽 모드 JAR는 각각 37개이며 클라이언트에는 화면용 모드, 서버에는 백업·진단 모드가 포함됨.
- CurseForge 메타데이터, JAR의 Forge·Minecraft·Architectury 의존 범위, Panda’s Falling Trees의 TreeChop 연동 이벤트 구현을 확인. 모든 배포 파일의 SHA-256·JAR 무결성과 실제 실행 이미지의 37개 모드 해시 검사 통과.
- `requireHoe=false`, `harvestInRadius=false`, `hungerLevel=NONE`, `experienceType=NONE`을 양쪽에 적용. 게임이 다시 저장한 설정에서도 값이 유지됨을 확인.
- 실제 `bash update.sh`로 이미지 빌드·정상 종료·약 83.7MiB의 월드 및 설정 백업·컨테이너 재생성 완료. 재시작 직후 플레이어 저장·발전과제·통계·권한 파일 5개의 해시가 업데이트 전과 일치함을 확인.
- 서버 `Done` 로그, 재시작 횟수 0, OOM 없음 확인. Windows 클라이언트의 메인 메뉴와 정품 서버 로그인·기존 월드 진입 성공. Falling Trees 클라이언트/서버 설정 동기화 로그 확인 후 로그아웃.
- TreeChop 사운드 중복 등록과 PandaLib refmap 경고는 출력되지만 부팅·접속은 성공. 기존 Industrial Foregoing 발전과제 오류는 동일하게 남아 있음.
- 나무가 쓰러지는 장면과 수확·재파종 동작을 직접 플레이하여 검증하거나 모든 모드 작물·나무 조합을 검사한 것은 아님.
